    ...
    try {
      ...
      setError(
        caught instanceof ApiError
          ? caught.message
          : "Could not load this cooking match.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const createRecipes = async () => {
    ...
    try {
      ...
      setRecipeError(
        caught instanceof ApiError
          ? caught.message
          : "Could not create the complete recipes.",
      );
    } finally {
      setGenerating(false);
    }
      ...
      <View style={styles.sectionCard}>
        ...
        <View
          style={[
            ...
            width < 380 && styles.contributionsSmall,
          ]}
        >
          <ContributionPanel
            title="You bring"
            foods={match.yourContributions}
            tone="you"
          />
          <ContributionPanel
      ...
      {recipes.length === 0 ? (
            ...
            {match.possibleMeals.map((meal, index) => (
              <View key={meal.mealName} style={styles.previewRow}>
                ...
                <View style={styles.previewCopy}>
                  ...
                  <Text style={styles.previewCulture}>
                    {meal.culturalOrigin}
                  </Text>
                </View>
              </View>
        ...
        <View style={styles.recipeSection}>
          ...
          {recipes.map((recipe, recipeIndex) => (
            <View
              key={`${recipe.mealName}-${recipeIndex}`}
            ...
            >
              ...
              <View style={styles.recipeBody}>
                ...
                <View style={styles.recipeContributions}>
                  <View style={styles.recipeContribution}>
                    ...
                    <Text style={styles.recipeValue}>
                      {recipe.ingredientsFromYou.join(", ")}
                    </Text>
                  </View>
                  <View style={styles.recipeContribution}>
                    <Text style={styles.recipeLabel}>
                      {match.matchedUserName.split(" ")[0].toUpperCase()} USES
                    </Text>
                    <Text style={styles.recipeValue}>
                      {recipe.ingredientsFromThem.join(", ")}
                    </Text>
                  </View>
                </View>
                {recipe.optionalMissingIngredients.length > 0 ? (
                  <View style={styles.optionalBox}>
                    ...
                    <Text style={styles.optionalText}>
                      {recipe.optionalMissingIngredients.join(" · ")}
                    </Text>
                  </View>
                ) : null}
                ...
                {recipe.cookingInstructions.map((instruction, index) => (
                  <View
                    key={`${recipe.mealName}-step-${index}`}
