...
import {
  getCookingConnection,
  updateMeetingArrangement,
...
export default function ConnectionDetailsScreen() {
  ...
  const rawId = Array.isArray(params.connectionId)
  ...
  const save = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Plan saved",
        "Your cooking partner will see these meeting details.",
      );
    } catch (caught) {
      Alert.alert(
        "Could not save plan",
  ...
  if (loading)
    return (
      <View style={styles.center}>
        <ActivityIndicator color="#173124" size="large" />
      </View>
    );
  if (!connection)
    return (
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} style={styles.page}>
      <View style={styles.connectedBanner}>
        <Text style={styles.connectedEyebrow}>COOKING CONNECTION</Text>
        <Text style={styles.connectedTitle}>You’re connected!</Text>
        <Text style={styles.connectedCopy}>
          Contact each other and agree on a safe, suitable place and time.
        </Text>
      </View>
      <View style={styles.card}>
        <PersonSummary
          name={connection.otherUserName}
          bio={connection.otherUserBio}
          profileImageUrl={connection.otherUserProfileImageUrl}
          cultures={connection.otherUserFoodCultures}
        />
        <View style={styles.divider} />
        <Text style={styles.label}>CONTACT EMAIL</Text>
        <Pressable
          disabled={!connection.contactEmail}
          onPress={() =>
            connection.contactEmail &&
            void Linking.openURL(`mailto:${connection.contactEmail}`)
          }
        >
          <Text style={styles.email}>
            {connection.contactEmail || "Contact unavailable"}
          </Text>
        </Pressable>
      </View>
      <View style={styles.card}>
        ...
        <Text style={styles.help}>
          Either person can update this shared plan.
        </Text>
        ...
        <TextInput
          ...
          maxLength={200}
          placeholder="e.g. Ōtara Community Kitchen"
        ...
        <TextInput
          ...
          maxLength={120}
          placeholder="e.g. Saturday at 11:00 am"
          placeholderTextColor="#8A938D"
          style={styles.input}
        />
        ...
        <TextInput
          ...
          maxLength={1000}
          multiline
          placeholder="What should each person bring?"
          placeholderTextColor="#8A938D"
        ...
        >
          {saving ? (
            ...
            <Text style={styles.saveText}>Save shared plan</Text>
          )}
        </Pressable>
      </View>
      <View style={styles.safety}>
        ...
        <Text style={styles.safetyCopy}>
          Choose a public or familiar place, confirm dietary needs, and tell
          someone where you are meeting.
        </Text>
      </View>
    </ScrollView>
  );
